...
// envia os dados para o display SSD1306
...
// desenha um pixel no display SSD1306
...
// preenche o display SSD1306 com um valor (true ou false)
void ssd1306_fill(ssd1306_t *ssd, bool value)
{
...
// desenha um retângulo no display SSD1306
...
// Desenha um círculo no display SSD1306
// ssd   -> Ponteiro para a estrutura do display
// x0, y0 -> Coordenadas do centro do círculo
// radius -> Raio do círculo
// value  -> Estado do pixel (true = ligado, false = desligado)
// fill   -> Se true, preenche o interior do círculo
void ssd1306_circle(ssd1306_t *ssd, uint8_t x0, uint8_t y0, uint8_t radius, bool value, bool fill)
{
    // Inicializa as coordenadas para o algoritmo de círculo
    int8_t x = radius; // Começa na borda horizontal direita
    int8_t y = 0;      // Começa no topo do círculo
    int8_t err = 0;    // Erro inicial (padrão para o algoritmo)

    // Continua desenhando até que os pontos se encontrem no meio
    while (x >= y)
    {
        // Desenha os 8 pontos simétricos do círculo
        ssd1306_pixel(ssd, x0 + x, y0 + y, value); // Quadrante superior direito
        ssd1306_pixel(ssd, x0 + y, y0 + x, value); // Quadrante superior direito (vertical)
        ssd1306_pixel(ssd, x0 - y, y0 + x, value); // Quadrante superior esquerdo (vertical)
        ssd1306_pixel(ssd, x0 - x, y0 + y, value); // Quadrante superior esquerdo
        ssd1306_pixel(ssd, x0 - x, y0 - y, value); // Quadrante inferior esquerdo
        ssd1306_pixel(ssd, x0 - y, y0 - x, value); // Quadrante inferior esquerdo (vertical)
        ssd1306_pixel(ssd, x0 + y, y0 - x, value); // Quadrante inferior direito (vertical)
        ssd1306_pixel(ssd, x0 + x, y0 - y, value); // Quadrante inferior direito

        // Preenche o interior do círculo se necessário
        if (fill)
        {
            // Preenche as linhas horizontais entre os dois lados do círculo
            for (int8_t i = x0 - x; i <= x0 + x; ++i)
            {
                ssd1306_pixel(ssd, i, y0 + y, value); // Linha superior
                ssd1306_pixel(ssd, i, y0 - y, value); // Linha inferior
            }

            // Preenche as linhas verticais entre os dois lados do círculo
            for (int8_t i = x0 - y; i <= x0 + y; ++i)
            {
                ssd1306_pixel(ssd, i, y0 + x, value); // Linha superior (vertical)
                ssd1306_pixel(ssd, i, y0 - x, value); // Linha inferior (vertical)
            }
        }

        // Atualiza as coordenadas para o próximo ponto do círculo
        if (err <= 0)
        {
            y++;              // Move para o próximo ponto vertical
            err += 2 * y + 1; // Ajusta o erro para refletir o movimento
        }

        if (err > 0)
        {
            x--;              // Move para dentro no eixo X
            err -= 2 * x + 1; // Ajusta o erro para refletir o movimento
        }
    }
}
